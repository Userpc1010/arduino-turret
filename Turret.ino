...
float set_zero_rad;                  //Установка 0ля
int set_zero_degr;
float angle_z;

Servo A2_servo;                //Обьект класса сервы А2 пин ШИМа т.е. pwm 2
Servo A3_servo;

int A2_pos;                   //Здесь хранится позиция сервы
int A3_pos;

int last;                      //Компенсация 360град

// Используемые глобальные переменные и функции доступа для хранения общих данных 
// угола поворота датчика
...
void set_last_read_angle_data(unsigned long time, float z, float z_gyro) {
  last_read_time = time;
  last_z_angle = z;
...
//  TWBR = 12;  // 400 kbit/sec I2C speed for Pro Mini
  ...
  A2_servo.attach (2);                // контакты к которым подключена серва 1 и 2я
  A3_servo.attach (3);
...
{  
  ...
  if (readByte(MPU9250_ADDRESS, MPU9250_INT_STATUS) & 0x01) {  // check if data ready interrupt
   ...
   float unfiltered_gyro_angle_z = gz*dt + get_last_gyro_z_angle();
 ...
 /* 
  ...
  // Pass gyro rate as rad/s
    MadgwickQuaternionUpdate(-ax, ay, az, gx*pi/180.0f, -gy*pi/180.0f, -gz*pi/180.0f,  1/*my*/,  1/*-mx*/, 1/*mz*/);
...
if (!stab_on)
{
  int sto_p = 90;
    int sto_p1 = 70;
    A2_servo.write (sto_p);   //горизонтальная
    A3_servo.write (sto_p1); //вертикальная
    stab_on = false;
}
...

{
 ...
 A2_pos =  data_A3 + pos_Y_stab;
 
 A2_servo.write (A2_pos);


 A3_pos =  data_A2 + pos_X_stab;
 
 A3_servo.write (A3_pos);
...
//-----------------------------------------------------------------------------------------------------------------------------
  ...
  if ( time_out && currentMillis - previousMillis >= ttime_out )
  {
           sp_ResetAll();
...
//------------------------------------------------------------------------------------------------------------------------------
    ...
    delt_t = millis() - count;
    if ( false && delt_t > 500 ) { // update LCD once per half-second independent of read rate
   ...
   if(SerialDebug) {
    ...
    altitude = 44330 *(1.0f - pow((BMP280Pressure/1013.25f), 0.190284f));
 ...
 //   roll  *= 180.0f / PI;
    ...
    pitch *= 180.0f / PI;
    yaw   *= 180.0f / PI;
    yaw   += 7.6f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04 ПОМЕНЯЛ УГОЛ МАГНИТНОГО СКЛОНЕНИЯ Клалькулятор маг склонения полученый результат в град, мин, сек их в десятичные градусы.
    if(yaw < 0) yaw   += 360.0f; // Ensure yaw stays between 0 and 360
    roll  *= 180.0f / PI;
